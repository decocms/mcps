...
function renderToolsMarkdown(
  snapshots: Record<GoogleService, { scopes: string[]; tools: BackendTool[] }>,
): string {
  const services = Object.keys(snapshots) as GoogleService[];
  const total = services.reduce(
    (acc, svc) => acc + new Set(snapshots[svc].tools.map((t) => t.name)).size,
    0,
  );
  ...
  const lines: string[] = [];
  lines.push("# Google Workspace — Tool Catalog");
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push(
    `${total} tools across ${services.length} services, gated behind ${allScopes.length} OAuth scopes.`,
  );
  lines.push("");
  lines.push(
  ...
  for (const service of services) {
    ...
    lines.push(`## ${service}`);
    lines.push("");
    lines.push(`**Endpoint:** \`${BACKEND_MCPS[service]}\``);
    lines.push("");
    lines.push("**Scopes:**");
...
async function main() {
  ...
  const snapshotsForMarkdown: Record<
    GoogleService,
    { scopes: string[]; tools: BackendTool[] }
  > = {} as never;
  ...
  await writeFile(TOOLS_MD_PATH, renderToolsMarkdown(snapshotsForMarkdown));
  console.log(`\nWrote ${services.length} snapshots to ${OUT_DIR}`);
  console.log(`Wrote tool catalog to ${TOOLS_MD_PATH}`);
}
