...
export const zCreateUpdateDimensionRequestBody = z.object({
    ...
    ruleCollection: z.array(z.object({
        ...
        priority: z.int().register(z.globalRegistry, {
            description: 'Sequential evaluation priority for the rule. Lower numbers are evaluated first. Rules must be ordered: bypass (`effectType: 0`) first, then deny (`effectType: 1`), then sequential workflow (`effectType: 2`). Priorities are assigned sequentially across all rules.'
        }),
        trigger: z.object({
            condition: z.object({
