...
4. CRÍTICO p/ o preview funcionar: o dev server do DAEMON do sandbox roda o script \`dev\` **sem disparar o lifecycle \`predev\`** — então os generates do deco (blocks/sections/loaders/schema/invoke) TÊM que estar DENTRO do próprio \`dev\`, não só num \`predev\` separado. O \`vite dev\` sozinho só gera o routeTree (via plugin tanstackStart) — NÃO gera os manifests/blocks do deco; sem eles o dev serve o placeholder "No web page". Configure AMBOS: \`"predev": "npm run generate:blocks && npm run generate:sections && npm run generate:loaders && npm run generate:schema && npm run generate:invoke && tsr generate"\` **E** \`"dev": "npm run predev && vite dev"\`. Assim, como quer que o daemon invoque o \`dev\`, os generates rodam antes do vite (é o que faz o preview do sandbox renderizar de verdade).
