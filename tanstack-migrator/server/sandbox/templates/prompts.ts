...
4. CRÍTICO p/ o preview do sandbox renderizar: o daemon do sandbox roda \`vite dev\` DIRETO (não dispara o \`predev\`). No \`vite dev\`, só são regenerados: **routeTree** (plugin tanstackStart) e **blocks + schema/meta** (decoVitePlugin, no configureServer). **sections, loaders e invoke NÃO são regenerados no dev** — se ficarem gitignored, somem no clone do daemon → o app não resolve as seções/loaders → preview serve "No web page". Portanto: rode os generates uma vez (\`npm install @decocms/start tsx && npm run predev\` ou \`npm run build\`) e **COMITE os gens que o dev NÃO regenera**: \`src/server/cms/sections.gen.ts\`, \`src/server/cms/loaders.gen.ts\`, \`src/server/invoke.gen.ts\` (+ \`src/*/manifest.gen.ts\` se houver). NÃO comite o que o dev regenera (deixe gitignored): \`src/routeTree.gen.ts\`, \`src/server/cms/blocks.gen.*\`, \`src/server/admin/*.gen.*\`. Ajuste o \`.gitignore\` pra refletir isso (troque um \`src/server/cms/*.gen.*\` amplo por só \`blocks.gen.*\`). Mantenha também \`"predev"\` + \`"dev": "npm run predev && vite dev"\` (ajuda o dev manual/local).
5. Checkpoint final. ANTES do commit, garanta que \`node_modules\`/artefatos NUNCA vão pra branch (node_modules commitado carrega deps de versão errada — ex: vite diferente do package.json — e quebra o dev no sandbox): \`grep -qxF 'node_modules/' .gitignore 2>/dev/null || printf '\\nnode_modules/\\ndist/\\n.wrangler/\\n.tanstack/\\n.vite/\\n' >> .gitignore; git rm -r --cached --quiet node_modules dist .wrangler .tanstack .vite 2>/dev/null || true\`. Depois: \`git add -A && git commit -m "feat: tanstack migration (script output + predev)" && git push -u -f origin ${branch}\` (a branch é gerenciada pela migração — o force é seguro).
