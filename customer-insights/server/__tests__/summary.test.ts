...
const mockGetSnapshot = mock(() => Promise.resolve(null as any));
...
// Importar as funções helper depois dos mocks (dynamic import para garantir que mocks são aplicados)
const {
  clean,
  determineStatus,
  ...
  createSummaryTool,
} = await import("../tools/summary.ts");

const mockEnv = {} as any;
createSummaryTool(mockEnv);
...
describe("formatBillingSection()", () => {
  it("deve calcular métricas de billing corretamente", () => {
    ...
    // avgMonthly = totalAmount / distinctMonths = 4700 / 3 = 1566.67
