...
const { createUploadCsvTool } = await import("../tools/upload-csv.ts");
createUploadCsvTool({} as any);
...
describe("upload_csv", () => {
  ...
  it("deve salvar sempre como billing.csv", async () => {
    globalThis.fetch = mock(() =>
      Promise.resolve(new Response("id,name\n1,Test", { status: 200 })),
    ) as any;
    ...
    await capturedExecute({
      context: {
        csv_url: "https://example.com/any-file.csv",
      },
    });

    // saveCsv deve ser chamado com "billing.csv"
    expect(mockSaveCsv).toHaveBeenCalledWith("billing.csv", expect.any(String));
  });
  ...
  it("deve propagar erro do reloadView gracefully", async () => {
    ...
    const result = await capturedExecute({
      context: { csv_url: "https://example.com/bad.csv" },
    });
  ...
  it("deve rejeitar URLs privadas (SSRF protection)", async () => {
    const result = await capturedExecute({
      context: { csv_url: "https://localhost/data.csv" },
    });
  ...
  it("deve rejeitar URLs HTTP (apenas HTTPS permitido)", async () => {
    const result = await capturedExecute({
      context: { csv_url: "http://example.com/data.csv" },
    });
