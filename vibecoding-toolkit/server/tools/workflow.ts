...
function parseAtRef(
    ref: `@${string}`,
): { type: "step" | "input"; id?: string; path?: string } {
...
const StepSchema = z.object({
    ...
    status: z.enum(["pending", "running", "completed", "failed"]).default(
        "pending",
    ),
            ...
            for (const step of steps) {
                ...
                if (stepDef.inputSchema) {
                    ...
                    if (!inputValidation.valid) {
                        const errorMessage =
                            `Step '${stepDef.name}' input validation failed after ref resolution: ${
                                inspect(inputValidation.errors)
                            }`;
                ...
                if (typeof step.execute === "string") {
                    ...
                    const result = stepCtx.dump(
                        stepCtx.unwrapResult(stepCallHandle),
                    );
                    ...
                    const connection = proxyConnectionForId(
                        step.execute.connectionId,
                        {
                    ...
                    const result = await env.INTEGRATIONS
