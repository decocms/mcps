...
const config = {
  ...
  defaultWorkflow: process.env.DEFAULT_WORKFLOW || "default-agent-loop",
  conversationWorkflow: process.env.CONVERSATION_WORKFLOW || "conversation",
};
...
async function publishEvent(
  ...
  if (!eventBusConnectionId) return;
...
async function main() {
  ...
  server.registerTool(
    "WORKFLOW_START",
    {
      ...
      description:
        "Start a workflow execution. Returns task ID for tracking. Default: default-agent-loop (command mode)",
      inputSchema: z.object({
        workflowId: z
          .string()
          .optional()
          .describe("Workflow to execute (default: default-agent-loop)"),
        input: z
          .record(z.string(), z.any())
          .describe("Workflow input (e.g. { message, history })"),
        source: z
          .string()
          .optional()
    ...
    async (args) => {
      ...
      const result = await startWorkflow(
        workflowId || config.defaultWorkflow,
        input,
        source || "cli",
        chatId,
      );
  ...
  console.error(`
╔══════════════════════════════════════════════════════════════╗
║                    PILOT v${PILOT_VERSION}                              ║
╠══════════════════════════════════════════════════════════════╣
║  Workflow-based AI agent for the MCP Mesh                    ║
║                                                              ║
║  Execution:                                                  ║
║  • WORKFLOW_START     - Start any workflow                   ║
║  • MESSAGE            - Smart routing (conv or command)      ║
║  • CONVERSATION_START - Start long-running thread            ║
║  • CONVERSATION_END   - End active conversation              ║
║                                                              ║
║  Tasks:                                                      ║
║  • TASK_GET/RESULT/LIST/CANCEL/STATS                         ║
║                                                              ║
║  Workflows:                                                  ║
║  • WORKFLOW_LIST/GET/CREATE                                  ║
║                                                              ║
