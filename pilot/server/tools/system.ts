 ...
 * Ensures the path is either exactly the allowed path or starts with it followed by a path separator
 */
function isPathAllowed(path: string): boolean {
  ...
  return ALLOWED_PATHS.some((allowed) => {
    ...
    // Starts with allowed path followed by path separator
    const prefix =
      normalizedAllowed + (normalizedAllowed.endsWith("/") ? "" : "/");
