...
describe("isAllowedRedirectUri", () => {
  test("accepts decocms.com apex and arbitrary subdomains over https", () => {
    ...
    expect(
      isAllowedRedirectUri("https://graph-mcp.decocms.com/oauth/callback"),
    ).toBe(true);
  ...
  test("rejects non-loopback http", () => {
    expect(isAllowedRedirectUri("http://graph-mcp.decocms.com/cb")).toBe(false);
...
describe("assertAllowedRedirectUri", () => {
  test("passes for an allowed uri", () => {
    expect(() =>
      assertAllowedRedirectUri("https://graph-mcp.decocms.com/cb"),
    ).not.toThrow();
  });
