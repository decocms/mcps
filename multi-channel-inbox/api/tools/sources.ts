...
export const removeSourceTool = (env: Env) =>
  createTool({
    ...
    description: "Remove a monitored source from the inbox.",
    inputSchema: z.object({
      id: z.string().describe("The source ID to remove"),
    }),
    outputSchema: z.object({ message: z.string() }),
    execute: async ({ context }) => {
      await runSQL(env, "DELETE FROM inbox_source WHERE id = ?", [context.id]);
      return { message: `Source ${context.id} removed` };
    },
  });
