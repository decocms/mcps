...
describe("isAllowedRedirectUri", () => {
  ...
  test("accepts deco.site apex and arbitrary subdomains over https", () => {
    ...
    expect(
      isAllowedRedirectUri("https://sites-openrouter.deco.site/oauth/callback"),
    ).toBe(true);
  });
  ...
  test("rejects non-loopback http", () => {
    expect(isAllowedRedirectUri("http://sites-openrouter.deco.site/cb")).toBe(
      false,
    );
...
describe("assertAllowedRedirectUri", () => {
  test("passes for an allowed uri", () => {
    expect(() =>
      assertAllowedRedirectUri("https://sites-openrouter.deco.site/cb"),
    ).not.toThrow();
  });
