 ...
 * OpenRouter as the `callback_url`. OpenRouter delivers the authorization
 * `code` to whatever host that URL points at, so if the origin is ever
 * attacker-influenced (spoofed Host header, an injected query/redirect
 * param, a misconfigured route) the code — and the API key minted from it —
 * leaks to that host.
 *
 * We close this by refusing any callback URL whose host isn't one of our
