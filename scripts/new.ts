...
/**
 * Writes a local .env with a freshly generated AUTH_TOKEN.
 *
 * Every MCP wraps its handler in `withAuth`, which reads AUTH_TOKEN at
 * startup and throws when it is absent. Generating it here means `bun run dev`
 * works on the first try without anyone being tempted to strip the wrapper.
 *
 * .env is gitignored — the value never leaves the machine, and production gets
 * its own secret.
 */
async function writeLocalAuthToken(destPath: string): Promise<string> {
  const token = randomBytes(32).toString("hex");
  await writeFile(
    join(destPath, ".env"),
    `# Local development secret, generated by scripts/new.ts.\n` +
      `# Production uses a different value — see .env.example.\n` +
      `AUTH_TOKEN=${token}\n`,
    "utf-8",
  );
  return token;
...
async function createMinimalMCP(options: CreateOptions): Promise<void> {
  ...
  const authToken = await writeLocalAuthToken(destPath);
  console.log("✓ Generated .env with a local AUTH_TOKEN");
  ...
  console.log("");
  console.log("Authentication (already wired, do not remove):");
  console.log(
    "  server/main.ts wraps the handler in withAuth() — the server will not",
  );
  console.log(
    "  start without AUTH_TOKEN, and CI fails if the wrapper is removed.",
  );
  console.log(`  local AUTH_TOKEN: ${authToken}`);
  console.log(
    "  production: provision AUTH_TOKEN as a secret before deploying",
  );
  console.log("");
...
async function createMcpAppMCP(options: CreateOptions): Promise<void> {
  ...
  console.log("");
  // The mcp-app template lives in a separate repository, so it cannot be
  // pre-wired from here. check-auth.ts will fail CI until this is done.
  console.log("⚠️  Authentication is NOT wired yet for the mcp-app template.");
  console.log("  Wrap the handler in api/main.ts before opening a PR:");
  console.log('    import { withAuth } from "@decocms/mcps-shared/auth";');
  console.log("    serve(withAuth(runtime.fetch));");
  console.log(`  local AUTH_TOKEN: ${authToken}`);
  console.log("");
  console.log("Next steps:");
  console.log(`  cd ${name}`);
