...
function scoreModelForTask(
  ...
  if (prioritize === "cost") {
    ...
    // Favor smaller, faster models (flash/lite variants)
    if (model.id.includes("flash-lite") || model.id.includes("flash-8b")) {
      score += 20;
    } else if (model.id.includes("flash")) {
      score += 15;
    }
  ...
  if (requirements.maxCostPer1MTokens !== undefined) {
    candidates = candidates.filter(
      (m) =>
        ...
        (requirements.maxCostPer1MTokens as number),
    );
  }

  if (requirements.minContextLength !== undefined) {
    candidates = candidates.filter(
      (m) => m.context_length >= (requirements.minContextLength as number),
    );
  }
...
function generateReasoning(
  ...
  // Quality/model tier
  if (model.id.includes("pro")) {
    reasons.push("High-quality flagship Gemini model");
  } else if (model.id.includes("flash")) {
    reasons.push("Fast and efficient Gemini model");
  }
  ...
  return reasons.join(". ") || "Good general-purpose Google Gemini model";
}
