...
export function registerTools(server: McpServer, storage: LocalFileStorage) {
  ...
  // These implement the OBJECT_STORAGE_BINDING for the Files plugin
  // ============================================================
  ...
  // GET_PRESIGNED_URL - Return file URL for local filesystem
  server.registerTool(
    "GET_PRESIGNED_URL",
    {
      title: "Get Presigned URL",
      description:
        "Get a URL for downloading a file. For local filesystem, returns a file:// URL or base64 data URL for images.",
      inputSchema: {
    ...
    withLogging("GET_PRESIGNED_URL", async (args): Promise<CallToolResult> => {
      try {
        const metadata = await storage.getMetadata(args.key);
        const absolutePath = storage.resolvePath(args.key);

        // For images, return data URL for preview in browser
        if (metadata.mimeType.startsWith("image/")) {
          const fileResult = await storage.read(args.key, "base64");
          const dataUrl = `data:${metadata.mimeType};base64,${fileResult.content}`;
        ...
        // For other files, return file:// URL
        const result = {
  ...
  server.registerTool(
    ...
    withLogging("PUT_PRESIGNED_URL", async (args): Promise<CallToolResult> => {
      // For local filesystem, we can't provide a presigned URL
      // Instead, provide the path and instructions
      const absolutePath = storage.resolvePath(args.key);

      const result = {
        url: `file://${absolutePath}`,
        expiresIn: 3600,
        // Note: The Files plugin may need to use write_file instead
  ...
  server.registerTool(
    "GET_ROOT",
    {
      ...
      description:
        ...
        "Used by Task Runner to know where to run beads operations.",
      inputSchema: {},
      annotations: { readOnlyHint: true },
    },
  ...
  // BEADS_INIT - Initialize Beads in the workspace
  server.registerTool(
    "BEADS_INIT",
    {
      title: "Initialize Beads",
      description:
        "Initialize Beads task tracking in the workspace. " +
        "Creates the .beads/ directory structure for task management.",
      inputSchema: {},
      annotations: { readOnlyHint: false },
    },
    withLogging("BEADS_INIT", async (): Promise<CallToolResult> => {
      try {
        const { mkdir } = await import("node:fs/promises");
        const { join } = await import("node:path");

        const beadsDir = join(storage.root, ".beads");
        ...
        const tasksPath = join(beadsDir, "tasks.json");
        try {
          await Bun.file(tasksPath).text();
        } catch {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error initializing beads: ${(error as Error).message}`,
            },
          ],
          isError: true,
        };
  ...
  server.registerTool(
    ...
    {
      title: "List Tasks",
      description: "List all tasks from the .beads/tasks.json file.",
      inputSchema: {
        status: z
    ...
    withLogging("TASK_LIST", async (args): Promise<CallToolResult> => {
      try {
        const { join } = await import("node:path");
        const tasksPath = join(storage.root, ".beads", "tasks.json");

        let tasksData: { tasks: TaskData[] } = { tasks: [] };
        try {
  ...
  server.registerTool(
    ...
    {
      title: "Create Task",
      description: "Create a new task in .beads/tasks.json.",
      inputSchema: {
    ...
    withLogging("TASK_CREATE", async (args): Promise<CallToolResult> => {
      try {
        const { join } = await import("node:path");
        const tasksPath = join(storage.root, ".beads", "tasks.json");
  ...
  server.registerTool(
    ...
    {
      title: "Update Task",
      description: "Update an existing task in .beads/tasks.json.",
      inputSchema: {
        ...
        threadId: z.string().optional().describe("Chat thread ID for this task"),
      },
      annotations: { readOnlyHint: false },
    },
    withLogging("TASK_UPDATE", async (args): Promise<CallToolResult> => {
      try {
        const { join } = await import("node:path");
        const tasksPath = join(storage.root, ".beads", "tasks.json");
        ...
        try {
          ...
          return {
            content: [{ type: "text", text: "Error: No tasks file found" }],
            isError: true,
          };
        }
        ...
        if (args.threadId !== undefined) task.threadId = args.threadId;
  ...
  server.registerTool(
    ...
    {
      title: "Delete Task",
      description: "Delete a task from .beads/tasks.json.",
      inputSchema: {
        id: z.string().describe("Task ID to delete"),
      },
      annotations: { readOnlyHint: false, destructiveHint: true },
    },
    withLogging("TASK_DELETE", async (args): Promise<CallToolResult> => {
      try {
        const { join } = await import("node:path");
        const tasksPath = join(storage.root, ".beads", "tasks.json");
        ...
        const result = { success: true, deletedId: args.id };
        return {
          content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
          structuredContent: result,
        };
